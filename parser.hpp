...
int parseDefectMetadata(parameter * & params, const int numparam, defects_metadata & defects_sim)
{
	...
	if (defects_sim.defect_flag==DEFECT_LOCAL)
	{
		...
		defects_sim.num_local_defect_output = MAX_OUTPUTS;

		COUT << "Parsing Local Defect parameters"<<endl<<endl;

		parseParameter(params, numparam, "tStart", defects_sim.tStart);
		if ( !isfinite(defects_sim.tStart))
		{
			COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": tStart in local defects not set properly!" << endl;
	#ifdef LATFIELD2_HPP
			parallel.abortForce();
	#endif
		}

		parseParameter(params, numparam, "tEnd", defects_sim.tEnd);
		if ( !isfinite(defects_sim.tEnd))
		{
			COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": tEnd in local defects not set properly!" << endl;
	#ifdef LATFIELD2_HPP
			parallel.abortForce();
	#endif
		}

		parseParameter(params, numparam, "eraA", defects_sim.eraA);
		if ( !isfinite(defects_sim.eraA))
		{
			COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": eraA in local defects not set properly!" << endl;
	#ifdef LATFIELD2_HPP
			parallel.abortForce();
	#endif
		}

	// 	parseParameter(params, numparam, "eraB", defects_sim.eraB);
	// 	if ( !isfinite(defects_sim.eraB))
	// 	{
	// 		COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": eraB in local defects not set properly!" << endl;
	// #ifdef LATFIELD2_HPP
	// 		parallel.abortForce();
	// #endif
	// 	}

		parseParameter(params, numparam, "tCoreGrowth", defects_sim.tCoreGrowth);
		if ( !isfinite(defects_sim.tCoreGrowth))
		{
			COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": tCoreGrowth in local defects not set properly!" << endl;
	#ifdef LATFIELD2_HPP
			parallel.abortForce();
	#endif
		}

		parseParameter(params, numparam, "coreGrowthIndexA", defects_sim.coreGrowthIndexA);
		if ( !isfinite(defects_sim.coreGrowthIndexA))
		{
			COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": coreGrowthIndexA in local defects not set properly!" << endl;
	#ifdef LATFIELD2_HPP
			parallel.abortForce();
	#endif
		}

		parseParameter(params, numparam, "coreGrowthIndexB", defects_sim.coreGrowthIndexB);
		if ( !isfinite(defects_sim.coreGrowthIndexB))
		{
			COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": eraB in local defects not set properly!" << endl;
	#ifdef LATFIELD2_HPP
			parallel.abortForce();
		...
		parseParameter(params, numparam, "sigma_loc", defects_sim.sigma_loc);
		if ( !isfinite(defects_sim.sigma_loc))
		{
			COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": sigma_loc in local defects not set properly!" << endl;
	#ifdef LATFIELD2_HPP
			parallel.abortForce();
	#endif
		}

		parseParameter(params, numparam, "lambda_loc", defects_sim.lambda_loc);
		if ( !isfinite(defects_sim.lambda_loc))
		{
			COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": lambda_loc in local defects not set properly!" << endl;
	#ifdef LATFIELD2_HPP
			parallel.abortForce();
	#endif
		}
		...
		parseParameter(params, numparam, "local_defect_statistics_redshifts", defects_sim.z_local_defects, defects_sim.num_local_defect_output);
			...
			if (defects_sim.num_local_defect_output > 0)
				...
				parseParameter(params, numparam, "numberEMconsOutputs", defects_sim.numberEMconsOutputs);
		    if (defects_sim.numberEMconsOutputs < 0)
		    {
			    COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": numberEMconsOutputs in local defects not set properly!" << endl;
	    #ifdef LATFIELD2_HPP
			    parallel.abortForce();
	    #endif
		    }
